...
### Output Directory Guardrails

Workers produce two kinds of output:

#### Structured handoff (system-managed, unchanged)
- Handoff JSON: `.ai/handoff/<worker-tid>.json`
- Checkpoint JSON: `.ai/checkpoints/<worker-tid>.json`
- Dispatch JSON: `.ai/handoff/dispatch-<worker-tid>.json`

#### Non-code artifacts
Any document, report, or analysis created by BOTS workers goes in `.ai/bots/<type>/`:

| Type | Path | Examples |
|------|------|----------|
| reports | `.ai/bots/reports/` | Analysis summaries, STUMPED/STUCK reports, audit results |
| docs | `.ai/bots/docs/` | Generated documentation, guides, READMEs |
| analysis | `.ai/bots/analysis/` | Deep dives, pattern reports, investigations |
| proposals | `.ai/bots/proposals/` | Architecture proposals, design plans, RFCs |
| designs | `.ai/bots/designs/` | UI mockups, wireframes, system diagrams |

**Rules:**
1. Code goes in the codebase — `.ts`, `.py`, etc. belong in the project source tree, never `.ai/bots/`
2. Non-code artifacts go in `.ai/bots/<type>/` — reports, analysis, docs, proposals, designs
3. New types on demand — `mkdir -p .ai/bots/<new-type>/` if nothing fits, document in handoff
4. Naming: `<worker>-<job-id>-<YYYYMMDD>.<ext>` (e.g., `analyst-job042-20260224.md`)
5. Handoff JSON stays separate — `.ai/handoff/` is for structured machine-readable data only
